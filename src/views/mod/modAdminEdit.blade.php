...
@section('title')
    Mod Manager &rarr; Edit &rarr; {{ $mod->id }}
@endsection


@section('content-header')
    <h1>Editing: {{ $mod->name }}</h1>
    <ol class="breadcrumb">
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                <div class="box-body table-responsive">
                    <form method="POST" action="{{ route('mod.update', $mod) }}">
                        @csrf
                        @method('PATCH')
                        <div class="form-group">
                            <label for="name" class="form-label">Name</label>
                            <input type="text" id="name" name="name" value="{{ $mod->name }}" class="form-control" />
                            <p class="text-muted small">Name of Mod.</p>
                        </div>

                        <div class="form-group">
                            <label for="desc" class="form-label">Description</label>
                            <input type="text" id="desc" name="description" value="{{ $mod->description }}" class="form-control" />
                            <p class="text-muted small">Description of the mod on the user's side.</p>
                        </div>

                        <div class="form-group">
                            <label for="vers" class="form-label">Version</label>
                            <input type="text" id="vers" name="version" value="{{ $mod->version }}" class="form-control" />
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="category" name="category_id">
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" @if($mod->category_id === $category->id) selected @endif>{{ $category->title }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="path" name="path" value="{{ $mod->path }}" class="form-control" />
                            <p class="text-muted small">Path for the mod to extract to on the users server. Do not include /home/conatiner.</p>
                        </div>

                        <div class="form-group">
                            <label for="link" class="form-label">Link</label>
                            <input type="text" id="link" name="link" value="{{ $mod->link }}" class="form-control" />
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="author" name="author" value="{{ $mod->author }}" class="form-control" />
                            <p class="text-muted small">The author of the mod.</p>
                        </div>

                        <div class="form-group">
                            <label for="folder" class="form-label">Folder name</label>
                            <input type="text" id="folder" name="foldername" value="{{ $mod->foldername }}" class="form-control" />
                    ...
                    <form action="{{ route('mod.destroy', $mod) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="button" id="function-delete" class="btn btn-danger btn-sm" style="float: right; margin: 5px;">Delete</button>
                    </form>
                </div>
            </div>
        </div>
